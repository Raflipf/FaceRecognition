...
const DummyData = {
    ...
    const sampleQueue = [
      {
        ...
        queueNumber: 1,
        examinationStartTime: new Date(
          today.getTime() + 8 * 60 * 60 * 1000 + 30 * 60 * 1000
        ).toISOString(),
      ...
      },
      {
        ...
        completionTime: new Date(
          today.getTime() + 9 * 60 * 60 * 1000 + 45 * 60 * 1000
        ).toISOString(),
        diagnosis: "ISPA (Infeksi Saluran Pernapasan Atas)",
        prescription:
          "Paracetamol sirup 3x1 sendok teh, OBH sirup 3x1 sendok teh",
      ...
      },
      {
        ...
        queueNumber: 1,
        examinationStartTime: new Date(
          today.getTime() + 10 * 60 * 60 * 1000 + 20 * 60 * 1000
        ).toISOString(),
      },
      {
        id: 4,
      ...
      },
      {
        ...
        status: "waiting",
        timestamp: new Date(
          today.getTime() + 11 * 60 * 60 * 1000 + 30 * 60 * 1000
        ).toISOString(), // 11:30 AM
        queueNumber: 1,
      },
      {
        id: 6,
  ...
  generateRandomPatient() {
    ...
    // Generate dummy embeddings as array of arrays with random floats
    const generateDummyEmbedding = () => {
      const length = 128; // example embedding length
      const embedding = [];
    ...
    // Generate 5 embeddings for 5 images
    const embeddings = [];
    for (let i = 0; i < 5; i++) {
      embeddings.push(generateDummyEmbedding());
    }

    return {
      name: `${firstName} ${lastName}`,
      nik: `317305${Math.floor(Math.random() * 1000000000)
        .toString()
        .padStart(10, "0")}`,
      ...
      emergencyContact: `Contact ${firstName} - 081${Math.floor(
